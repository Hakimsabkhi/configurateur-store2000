...
import { Devis } from "@/types/interfaces";
...
const DevisTable: React.FC<DevisTableProps> = ({
  ...
  const handleModify = (id: string) => {
    ...
    if (selectedDevis) {
      ...
      const cartItem = cartItems.find(
        (item) => item.devisNumber === selectedDevis.DevisNumber
      );
      if (cartItem) {
        const updatedTotalHT =
          selectedDevis.totalPrice * selectedDevis.multiplier;
        ...
        dispatch(
          updateCartItem({
            ...
            optionMotorisationSelected:
              selectedDevis.optionMotorisationSelected,
            optionTelecomandeSelected:
              selectedDevis.optionTelecomandeSelected || "",
            optionInterrupteurSelected:
              selectedDevis.optionInterrupteurSelected || "",
  ...
  const handleAddToCart = (devisItem: Devis) => {
    const isAlreadyInCart = cartItems.some(
      (item) => item.devisNumber === devisItem.DevisNumber
    );
    ...
    if (isAlreadyInCart) {
      ...
      if (toastId.current === undefined || !toast.isActive(toastId.current)) {
        toastId.current = toast.error(
          `Devis Numéro: ${devisItem.DevisNumber} est déjà ajouté au panier.`
        );
      }
    } else {
      ...
      // Dispatch action with all the necessary data from devisItem
      dispatch(
        addToCart({
          id: devisItem._id,
          ...
          lameSelected: devisItem.lameSelected,
          dimensions: devisItem.dimensions,
          selectedColor: {
            coulisse: devisItem.selectedCoulisseColor,
            tablier: devisItem.selectedTablierColor,
            lameFinale: devisItem.selectedLameFinaleColor,
          },
          poseInstalled: devisItem.poseInstalled,
          manoeuvreSelected: devisItem.manoeuvreSelected,
          commandeManualSelected: devisItem.commandeManualSelected || "",
          optionMotorisationSelected: devisItem.optionMotorisationSelected,
          optionTelecomandeSelected: devisItem.optionTelecomandeSelected || "",
          optionInterrupteurSelected:
            devisItem.optionInterrupteurSelected || "",
          sortieDeCableSelected: devisItem.sortieDeCableSelected || "",
        })
      );
      toast.success(
        `Devis Numéro: ${devisItem.DevisNumber} a été ajouté au panier.`
      );
    }
  };
              ...
              <tr>
                ...
                <td className="py-2 px-4 border-b text-sm font-bold border-black">
                  {`${devisItem.totalPrice.toFixed(2)} * ${
                    devisItem.multiplier
                  } = ${(devisItem.totalPrice * devisItem.multiplier).toFixed(
                    2
                  )}€`}
                </td>
                <td className="py-2 px-4 border-b text-sm font-bold border-black">
                  {(devisItem.totalPrice * devisItem.multiplier * 1.2).toFixed(
                    2
                  )}
                  €
                </td>
