...
import { WarningPopupProps } from "../types/interfaces";
...
const WarningPopup: React.FC<WarningPopupProps> = ({ message, onClose }) => {
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-80 flex justify-center items-center z-1000 transition-opacity duration-300 ease-in-out opacity-100">
      <div className="bg-[#e46148] rounded-[10px] text-center w-[30%] max-md:w-[80%] h-fit flex gap-1 flex-col justify-center px-2 py-6 transition-transform duration-300 ease-in-out transform translate-y-0">
        ...
        <p className="text-lg max-md:text-sm">{message}</p>
      </div>
    </div>
  );
};
